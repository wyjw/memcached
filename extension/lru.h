...
#define LRU_ITEM_size   (sizeof(lru_item))

#define LRU_ITEM_key(item) ((char*)((item)->data))    /* terminal null */

#define LRU_ITEM_data(item) (char*) ((item)->data) + (item)->nkey + 1
...
*/
int lru_item_get(lru *l, const char *key, const size_t nkey, char *buf, const size_t nbuf, size_t *nvalue);

/* 0 success , 1 failed */
int lru_item_set(lru *l, const char *key, const size_t nkey, const char *value, const size_t nvalue);

/* 0 hit, 1 miss */
int lru_item_delete(lru *l, const char *key, const size_t nkey);

#define stat_reset(l) do { \
    memset(&(l->stat), 0, sizeof(struct lru_stat)); \
} while(0)

void lru_stat_print(lru *l, char *buf, const int nbuf);
